...
public class BackupRequestLBHttpSolrServer extends LBHttpSolrServer {
  ...
  public Rsp request(Req req) throws SolrServerException, IOException {
    ArrayBlockingQueue<Future<RequestTaskState>> queue = new ArrayBlockingQueue<Future<RequestTaskState>>(maximumConcurrentRequests);
    ...
    for (String serverStr : req.getServers()) {
      ...
      if (returnedRsp == null) { 
        ...
        if (log.isInfoEnabled()){
          String servers = "";
          for (String serverName: req.getServers()){
            servers += " " + serverName;
          }
          log.info("Servers :{} did not respond before the backUpRequestDelay time of {} elapsed", servers,backUpRequestDelay);
        }
        continue;
      }
      inFlight--;
    ...
    if (returnedRsp == null || returnedRsp.stateDescription == TaskState.ServerException) {
      ...
      for (ServerWrapper wrapper : skipped) {
        ...
        if (returnedRsp == null) {
          if (log.isInfoEnabled()){
            String servers = "";
            for (ServerWrapper serverName: skipped){
              servers += " " + serverName;
            }
            log.info("Servers :{} did not respond before the backUpRequestDelay time of {} elapsed", servers,backUpRequestDelay);
          }
